...
def load_github_config():
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "GitHub config.json must contain a JSON object."
        )
    ...
    if not username:
        raise RuntimeError(
            "GitHub username is missing from config.json."
        )

    if not token:
        raise RuntimeError(
            "GitHub token is missing from config.json."
        )
...
def get_github_token(
    ...
    if (
        ...
        raise RuntimeError(
            (
                "Requested GitHub account does not match "
