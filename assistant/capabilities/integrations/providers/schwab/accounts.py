...
def schwab_portfolio_performance(
    ...
    for item in (
        ...
        for position in positions:
            ...
            raw_day_profit_loss = position.get(
                "currentDayProfitLoss"
            )
            raw_day_profit_loss_percent = position.get(
                "currentDayProfitLossPercentage"
            )
            day_profit_loss = float(raw_day_profit_loss or 0.0)
            day_profit_loss_percent = float(raw_day_profit_loss_percent or 0.0)
            ...
            net_quantity = quantity - short_quantity

            # Schwab sometimes returns zero/tiny current-day P/L placeholders
            # while instrument.netChange contains the actual quoted daily move.
            if (
                abs(day_profit_loss) < 0.01
                and abs(day_profit_loss_percent) < 0.0001
                and abs(net_change) > 0.0
                and abs(net_quantity) > 0.0
            ):
                day_profit_loss = net_change * net_quantity
                previous_price = (market_value / net_quantity) - net_change
                if previous_price:
                    day_profit_loss_percent = net_change / previous_price * 100.0
